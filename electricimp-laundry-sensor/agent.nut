...
/******************** APPLICATION CODE ********************/
const PUBKEY = "pub-c-********-****-****-****-************";                // set your publish key
const SUBKEY = "sub-c-********-****-****-****-************";                // set your subscribe key
...
device.on("washingmachine", function(status) {
    if (status == "finished") {
        pubnub.publish(growlChannel, {message= "Washing machine finished running.", timestamp=time()});
    }
